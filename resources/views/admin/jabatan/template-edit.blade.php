...
@section('page-description', 'Edit jabatan dalam sistem E-Inspection')
...
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('admin.jabatan.update', $jabatan->id) ?? '#' }}" method="POST" id="jabatanForm">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="card bg-light">
                                ...
                                <div class="card-body">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            <h6 class="mb-0" id="previewName">{{ $jabatan->name }}</h6>
                                            <small class="text-muted">Jabatan akan diperbarui</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
document.getElementById('name').addEventListener('input', function() {
    ...
    if (nameValue) {
        ...
        previewName.textContent = '{{ $jabatan->name }}';
    }
});

// Reset form function
function resetForm() {
    if (confirm('Apakah Anda yakin ingin mereset form? Semua perubahan akan dikembalikan ke data asli.')) {
        document.getElementById('name').value = '{{ $jabatan->name }}';
        document.getElementById('previewName').textContent = '{{ $jabatan->name }}';
...
document.getElementById('jabatanForm').addEventListener('submit', function(e) {
    ...
    if (!nameInput.value.trim()) {
        ...
        submitBtn.innerHTML = '<i class="bi bi-check-circle me-1"></i> Update Jabatan';
        nameInput.focus();
        return false;
    }
